...
return function(config)
  return {
    ...
    getLSPConfigMap = function()
      local list = {
        tsserver = require("jvim.lsp.config.typescript"),
        cssls = require("jvim.lsp.config.css"),
        emmet_ls = require("jvim.lsp.config.emmet"),
        html = require("jvim.lsp.config.html"),
      }
      if config.linter == "eslint" then
        list.eslint = require("jvim.lsp.config.eslint")
      end
      if config.tailwindcss then
        list.tailwindcss = require("jvim.lsp.config.tailwindcss")
      end
      if config.vue then
        list.volar = require("jvim.lsp.config.volar")
      end
      if config.prisma then
        list.prismals = require("jvim.lsp.config.prismals")
      end
