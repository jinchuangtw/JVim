...
local cfg = pRequire("jvim").config.cmp

local pathUtils = require("jvim.utils.path")
...
pRequire("luasnip.loaders.from_lua").load({
  paths = pathUtils.join(pathUtils.getConfig(), "lua", "jvim", "cmp", "snippets", "lua"),
})
pRequire("luasnip.loaders.from_vscode").lazy_load({
  paths = pathUtils.join(pathUtils.getConfig(), "lua", "jvim", "cmp", "snippets", "vscode"),
})
